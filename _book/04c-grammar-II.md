...
As an example, in our visualization of income and education there is a slight right skew to `med_house_income`. The graphic doesn't justify a logarithmic transformation, but may benefit from a square root transformation. We can apply this with `scale_x_sqrt()`.
...
This is an over correction. The slight upward arch in the original plotting is now inverted. Nonetheless, I hope the point is made. 
...
To recreate the above plot but faceting by county, we would add `facet_wrap(vars(county))` as a layer to our plot.
