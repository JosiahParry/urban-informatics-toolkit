...
```{r, echo=FALSE}
library(patchwork)
gd <- ggplot(acs_housing, aes(med_yr_moved_inraw)) +
  geom_density()

gh <- ggplot(acs_housing, aes(med_yr_moved_inraw)) +
  geom_histogram(bins = 10)
gd|gh
```
...
gba_edu <- ggplot(gba_acs, aes(bach, county)) +
  ggridges::geom_density_ridges() +
  labs(title = "Greater Boston Area Educational Attainment",
       x = "",
       y = "") +
  scale_x_continuous(labels = scales::percent)

suffolk_edu <- filter(gba_acs, county == "Suffolk County") %>% 
  mutate(income_quintile = ntile(med_house_income, 5)) %>% 
  ggplot(aes(bach, y = as.factor(income_quintile))) +
  ggridges::geom_density_ridges() + 
  labs(#title = "Suffolk County Educational Attainment",
       subtitle = "Suffolk County tracts partitioned by income quintile",
       y = "Income Quintile",
       x = "Bachelor Degree Attainment Rate") +
  scale_x_continuous(labels = scales::percent)
